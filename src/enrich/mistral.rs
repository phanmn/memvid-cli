//! Mistral enrichment engine using Mistral Large.
//!
//! This engine uses the Mistral API to extract structured memory cards
...
/// The extraction prompt for Mistral
...
/// Mistral enrichment engine using Mistral Large with parallel processing.
...
impl MistralEngine {
    ...
    /// Run inference via Mistral API (blocking, thread-safe).
            ...
            if let (Some(k), Some(e), Some(s), Some(v)) = (kind, entity, slot, value) {
                if !e.is_empty() && !s.is_empty() && !v.is_empty() {
                    match MemoryCardBuilder::new()
                        ...
                        .engine("mistral:mistral-large", "1.0.0")
                        .build(0)
                    {
    ...
    pub fn enrich_batch(
        ...
        let client = self
            .client
            .as_ref()
            .ok_or_else(|| anyhow!("Mistral engine not initialized (init() not called)"))?
            .clone();
            ...
            contexts
                ...
                .map(|(i, ctx)| {
                    ...
                    match Self::run_inference_blocking(&client, &api_key, &model, &ctx.text) {
                        ...
                        Err(err) => {
                            warn!(
                                "Mistral inference failed for frame {}: {}",
                                ctx.frame_id, err
                            );
                            (ctx.frame_id, vec![])
                        }
...
impl EnrichmentEngine for MistralEngine {
    fn kind(&self) -> &str {
    ...
    fn init(&mut self) -> memvid_core::Result<()> {
        if self.api_key.is_empty() {
            return Err(memvid_core::MemvidError::EmbeddingFailed {
                reason: "MISTRAL_API_KEY environment variable not set".into(),
            });
        }
        let client = crate::http::blocking_client(Duration::from_secs(60)).map_err(|err| {
            memvid_core::MemvidError::EmbeddingFailed {
                reason: format!("Failed to create Mistral HTTP client: {err}").into(),
            }
        })?;
    ...
    fn enrich(&self, ctx: &EnrichmentContext) -> EnrichmentResult {
        ...
        let client = match self.client.as_ref() {
            ...
            None => {
                return EnrichmentResult::failed(
                    "Mistral engine not initialized (init() not called)".to_string(),
                )
            }
        };
        ...
        match Self::run_inference_blocking(client, &self.api_key, &self.model, &ctx.text) {
            Ok(output) => {
                debug!("Mistral output for frame {}: {}", ctx.frame_id, output);
                let cards = Self::parse_output(&output, ctx.frame_id, &ctx.uri, ctx.timestamp);
                EnrichmentResult::success(cards)
            }
            Err(err) => EnrichmentResult::failed(format!("Mistral inference failed: {}", err)),
        }
    }
}
